...
var MoveLine = function MoveLine(map, userOptions) {
    ...
    var brush = function brush() {
        ...
        markLines.forEach(function (line) {
        	map.addPoint(line.from.location);
        	map.addPoint(line.to.location);
    ...
    var mouseInteract = function mouseInteract() {
        ...
        map.camera.changed.addEventListener(function(e){
        	$(superMap.htmlMark).each(function(i,e){
        		var mark=document.getElementById("mark_"+e.id);
        		console.log(mark);
				var po=superMap.pointToPixel({"lng":e.lon,"lat":e.lat});
				$(mark).css({"left":po.x+"px","top":po.y+"px"});
			});
        	//animationFlag = false;
    ...
    var addMarkLine = function addMarkLine() {
        ...
        data.forEach(function (line, i) {
            markLines.push(new MarkLine({
                ...
                from: new Marker({
                    city: line.from.city,
                    location: {"lng":line.from.lnglat[0], "lat":line.from.lnglat[1]},
                ...
                to: new Marker({
                    city: line.to.city,
                    location: {"lng":line.to.lnglat[0], "lat":line.to.lnglat[1]},
                    color: line.to.color,
                    type:line.to.type,
                    speed:line.to.speed,
                    max:line.to.max,
